...
const CHAR_ACTION_FRAMES: Record<string, number> = {
  Idle: 8,
  Walking: 8,
  Jumping: 8,
  Attacking: 8,
...
const ACTIONS = Object.keys(CHAR_ACTION_FRAMES);
...
export class SpriteEngine {
  ...
  private charSprites: Record<string, Record<string, HTMLImageElement[]>> = {};
  private badges: Record<string, HTMLImageElement> = {};
  private entities: Entity[] = [];
  private spawnFrames: HTMLImageElement[] = [];
  ...
  public async loadAssets() {
    ...
    const charIds = ["char_1", "char_2", "char_3", "char_4", "char_5"];

    for (const charId of charIds) {
      this.charSprites[charId] = {};
      const basePath = `/images/characters/${charId}`;
      const prefix = `${charId}_`;

      for (const [action, count] of Object.entries(CHAR_ACTION_FRAMES)) {
        this.charSprites[charId][action] = [];
        for (let i = 0; i < count; i++) {
          ...
          promises.push(
            new Promise<void>((resolve) => {
              const img = new Image();
              img.onload = () => {
                this.charSprites[charId][action][i] = img;
                resolve();
              };
              img.onerror = () => resolve();
    ...
    // Default fallback sprite set
    this.sprites = this.charSprites["char_2"];

    // Spawn effect (shared, not per-character)
    this.spawnFrames = [];
    for (let i = 0; i < 8; i++) {
  ...
  public spawnForUser(username: string, badge?: string, color?: string, charId?: string) {
  ...
  private resize() {
    if (this.config.fitContainer) {
      this.canvas.width = this.canvas.parentElement?.clientWidth ?? this.canvas.width;
  ...
  private pickNextAction(e: Entity) {
    const available = this.config.allowedActions?.length ? this.config.allowedActions : ACTIONS;
  ...
  private update(dt: number) {
    ...
    this.entities.forEach((e) => {
      ...
      const maxFrames =
        e.currentAction === "Spawning" ? 8 : (CHAR_ACTION_FRAMES[e.currentAction] ?? 8);
  ...
  private draw() {
    ...
    sorted.forEach((e) => {
      ...
      const cid = e.characterId || "char_2";
      const charSet = this.charSprites[cid] || this.sprites;
      const charFrames = charSet[charAction];
      ...
      if (e.currentAction === "Spawning") {
        if (this.spawnFrames[e.currentFrame]) {
          this.ctx.save();
          this.ctx.imageSmoothingEnabled = false;
          this.ctx.drawImage(this.spawnFrames[e.currentFrame], drawX, drawY, dw, dh);
          this.ctx.restore();
        }
      }
